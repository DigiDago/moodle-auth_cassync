...
$string['auth_cassyncdescription'] = 'This method uses a CAS server (Central Authentication Service) to authenticate users in a Single Sign On environment (SSO). If the given username and password are valid according to CAS, Moodle creates a new user entry in its database, taking user attributes from CAS if required. On following logins only the username and password are checked. <br />Whith this method you can synchronize attributes directly from your CAS to Moodle profile fields. It’s particularly helpful if you can’t connect your LDAP with Moodle. By doing so, you will have the same result as if you had used the LDAP settings available in Moodle core, but using only your CAS.';
